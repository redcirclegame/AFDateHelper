...
public extension Date {
    ...
    */
    ...
    {
        if string.isEmpty {
            (self as NSDate).dynamicType.init()
            return
        }
        ...
        case .dotNet:
            ...
            (self as NSDate).dynamicType.init(timeIntervalSince1970: interval)
        ...
        case .iso8601(let isoFormat):
            ...
            if let date = formatter.date(from: string as String) {
                (self as NSDate).dynamicType.init(timeInterval:0, since:date)
            } else {
                (self as NSDate).dynamicType.init()
            }
